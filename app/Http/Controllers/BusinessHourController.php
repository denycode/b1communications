...
class BusinessHourController extends Controller
{
    ...
    public function getBusinessHours(Request $request,Ajax $ajax){
    	...
    	try {
            ...
            elseif ($tabid == 22){
                ...
                $dDaynights = collect($dDaynights)->map(function($x){ return (array) $x; })->toArray();
                $tabName = 'Day & Night Buttons';
                if($rType == 'pagination'){
                    $html = View::make('business_hours.daynight.tabs.list.table',['dDaynights' => $dDaynights,'tab' => $tabName])->render();
                }else{
                    $html = View::make('business_hours.daynight.tabs.list.index',['dDaynights' => $dDaynights,'tab' => $tabName])->render();
                }

                $tTotalDaynight = $cConnection->table('daynight')
                    ->whereIn('ext',$aAssocDaynight)
                    ->where('dmode','fc_description')
                    ->groupBy('ext')
                    ->count();

                $paginationhtml = View::make('business_hours.daynight.tabs.list.pagination-html',[
                    'total_records' => $tTotalDaynight,
                    'records' => $dDaynights,
                    'position' => $position,
                    'records_per_page' => $records_per_page,
                    'page' => $page,
                ...
                return $ajax->success()
                    ->appendParam('dDaynight', $dDaynights)
                    ->appendParam('html',$html)
                    ->appendParam('paginationHtml',$paginationhtml)
                    ->jscallback('load_ajax_tab')
                    ->response();
    ...
    public function timeGroupsDetails($enc_timegroup_id,Ajax $ajax){
        ...
        return view('business_hours.timegroups.form.timegroup-details',[
    ...
    public function timeconditionsDetails($enc_timecondition_id,Ajax $ajax){
        ...
        return view('business_hours.timeconditions.form.timecondition-detail',[
    ...
    public function dayNightEdit($enc_daynight_ext,Ajax $ajax){
        $connection = DatabaseConnection::setConnection(Auth::user()->organization->server);
        if(!$connection){
            return $ajax->success()
                ->jscallback()
        ...
        $daynight_ext = Crypt::decrypt($enc_daynight_ext);
        $dDayNights = $connection->table('daynight')->where('ext',$daynight_ext)->get();
        $data = [];
        $data['ext'] = $daynight_ext;
        foreach ($dDayNights as $dayNight){
            if($dayNight->dmode == 'day'){
                $data['day'] = $dayNight->dest;
            }else if($dayNight->dmode == 'night'){
                $data['night'] = $dayNight->dest;
            }else if($dayNight->dmode == 'fc_description'){
                $data['fc_description'] = $dayNight->dest;
            }else if($dayNight->dmode == 'night_recording_id'){
                $data['night_recording_id'] = $dayNight->dest;
            }else if($dayNight->dmode == 'day_recording_id'){
                $data['day_recording_id'] = $dayNight->dest;
            }
        }

        $pageTitle = 'Edit Day & Night Button';
        return view(Helper::OrgNotFound(Auth::user()->IsOrganizationAssigned,'business_hours.daynight.form.add'),[
    ...
    public function dayNightUpdate(Request $request,Ajax $ajax){
        $connection = DatabaseConnection::setConnection(Auth::user()->organization->server);
        if(!$connection){
            return $ajax->success()
                ->jscallback()
        ...
        $ext = $request->ext;
        if(isset($request->dmode)){
            $dmodes = $request->dmode;
            foreach ($dmodes as $dmode => $dest){
                $connection->table('daynight')
                    ->where('ext',$ext)
                    ->where('dmode',$dmode)
                    ->update(['dest' => $dest]);
            }
        ...
        return $ajax->success()
            ->jscallback()
            ->appendParam('redirect',true)
            ->message('Day & Night button update successfully')
            ->redirectTo(route('business_hours.index').'#tab=tab_22')
            ->response();
    }
}
?>
