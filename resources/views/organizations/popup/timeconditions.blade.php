...
<form class="ajax-Form" enctype="multipart/form-data" method="post" action="{!! URL::to('/organization/unassignedupdate') !!}">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="exampleInputEmail1">Unassigned Time Conditioons</label>
            <div class="m-b-30">
                <select name="grplist" class="form-control" id="multiselect">
                    <option value="">Select</option>
                    @foreach($tTimeconditions as $tTimecondition)
                        @if(!empty(trim($tTimecondition->timeconditions_id)))
                            <option
                            value="{{$tTimecondition->timeconditions_id}}::{{$tTimecondition->displayname}}"
                            >
                                {{$tTimecondition->displayname}}
                            </option>
                        @endif
                    @endforeach
                </select>
    ...
    <div class="form-actions pull-right depend">
        <input type="hidden" name="type" value="timecondition">
