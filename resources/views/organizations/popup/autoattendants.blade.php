...
<form class="ajax-Form" enctype="multipart/form-data" method="post" action="{!! URL::to('/organization/unassignedupdate') !!}">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="exampleInputEmail1">Unassigned Auto Attendants</label>
            <div class="m-b-30">
                <select name="grplist" class="form-control" id="multiselect">
                    <option value="">Select</option>
                    @foreach($aAutoattendants as $aAutoattendant)
                        @if(!empty(trim($aAutoattendant->id)))
                            <option
                                    value="{{$aAutoattendant->id}}::{{$aAutoattendant->name}}"
                            >
                                {{$aAutoattendant->name}}
                            </option>
                        @endif
    ...
    <div class="form-actions pull-right depend">
        <input type="hidden" name="type" value="autoattendant">
