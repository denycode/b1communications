...
<form class="ajax-Form" enctype="multipart/form-data" method="post" action="{!! URL::to('/organization/unassignedupdate') !!}">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="exampleInputEmail1">Unassigned Time Groups</label>
            <div class="m-b-30">
                <select name="grplist" class="form-control" id="multiselect">
                    <option value="">Select</option>
                    @foreach($tTimegroups as $tTimegroup)
                        @if(!empty(trim($tTimegroup->id)))
                            <option
                                    value="{{$tTimegroup->id}}::{{$tTimegroup->description}}"
                            >
                                {{$tTimegroup->description}}
                            </option>
                        @endif
                    @endforeach
                </select>
