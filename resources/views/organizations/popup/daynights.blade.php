...
<form class="ajax-Form" enctype="multipart/form-data" method="post" action="{!! URL::to('/organization/unassignedupdate') !!}">
    ...
    <div class="row">
        <div class="col-md-12">
            <label for="exampleInputEmail1">Unassigned Day & Night Buttons</label>
            <div class="m-b-30">
                <select name="grplist" class="form-control" id="multiselect">
                    <option value="">Select</option>
                    @foreach($dDaynights as $dDaynight)
                        @if(!empty(trim($dDaynight->ext)))
                            <option
                            value="{{$dDaynight->ext}}::{{$dDaynight->dest}}"
                            >
                                {{$dDaynight->dest}}
                            </option>
                        @endif
    ...
    <div class="form-actions pull-right depend">
        <input type="hidden" name="type" value="daynight">
